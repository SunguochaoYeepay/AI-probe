...
export const AI_PROMPTS = {
  // 页面名称提取提示词
  EXTRACT_PAGE_NAME: (userInput) => `请仔细分析用户输入，提取页面关键词。

用户输入：${userInput}

请按照以下步骤分析：
1. 仔细阅读用户输入内容
2. 识别用户想要分析的具体页面或功能模块
3. 提取核心关键词（去除"页面"、"访问量"、"的访问"、"分析"、"查询"等通用词汇）
4. 返回最能代表页面功能的关键词

重要提醒：
- 必须根据用户的实际输入内容提取，不要返回任何示例答案
- 如果用户输入中没有具体的功能模块，请返回null
- 如果用户输入的是通用描述（如"页面访问量"），请返回null
- 如果用户输入包含不当词汇，请返回null
